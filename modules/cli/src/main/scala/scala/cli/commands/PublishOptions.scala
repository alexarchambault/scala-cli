...
import scala.build.options.{BuildOptions, PostBuildOptions, PublishOptions => BPublishOptions}
    ...
    baseOptions.copy(
      ...
      notForBloopOptions = baseOptions.notForBloopOptions.copy(
        publishOptions = baseOptions.notForBloopOptions.publishOptions.copy(
          ...
          gpgSignatureId = signWith.map(_.trim).filter(_.nonEmpty)
        )
      )
    )
  }
