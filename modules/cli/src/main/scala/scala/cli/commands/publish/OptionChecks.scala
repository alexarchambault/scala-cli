...
object OptionChecks {
  ...
  // private def versionCheck(options: PublishSetupOptions) = OptionCheck(
  //   "version",
  //   "publish.version",
  //   pubOpt => pprint.err.log(pubOpt.version.nonEmpty || pubOpt.computeVersion.nonEmpty),
  //   () =>
  //     options.sharedPublish.version.toRight {
  ...
  private def userCheck(options: PublishSetupOptions) = OptionCheck.opt(
    ...
    _.repoUser,
    () =>
      options.sharedPublish.user.map(_.get()).map(u => ("env:PUBLISH_USER", Nil, Seq(SetSecret("PUBLISH_USER", u, force = true)))).toRight {
  ...
  private def pgpSecretKeyCheck(
    ...
    () => either {
      ...
      pubKeyOpt match {
        ...
        case None =>
          logger.message("Warning: no public key passed, not checking if the key needs to be uploaded to a key server.")
      }
      ...
      val setSecrets = Seq(SetSecret("PUBLISH_SECRET_KEY", secretKey, force = true)) ++ passwordSetSecret.toSeq
  ...
  private def urlCheck(
    ...
    () => {
      def ghUrlOpt = GitRepo.ghRepoOrgName(workspace, logger) match {
        case Left(err) =>
          logger.debug(s"Error when trying to get GitHub repo from git to get default project URL: $err, ignoring it.")
          None
        case Right((org, name)) =>
  ...
  private def scmCheck(
    ...
    () => {
      def ghVcsOpt = GitRepo.ghRepoOrgName(workspace, logger) match {
        case Left(err) =>
          logger.debug(s"Error when trying to get GitHub repo from git to get default project VCS: $err, ignoring it.")
          None
        case Right((org, name)) =>
