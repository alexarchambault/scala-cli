...
object Package extends ScalaCommand[PackageOptions] {
  ...
  def libraryJar(build: Build.Successful): Array[Byte] = {
  ...
  private def sourceJar(build: Build.Successful, defaultLastModified: Long): Array[Byte] = {
    ...
    pprint.stderr.log(build.sources.paths)
    pprint.stderr.log(build.sources.inMemory)
    ...
    def fromGeneratedSources = build.sources.inMemory.iterator.flatMap { inMemSource =>
      ...
      Iterator((inMemSource.generatedRelPath, inMemSource.generatedContent.getBytes(StandardCharsets.UTF_8), lastModified))
    }
