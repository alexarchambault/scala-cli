...
object PreprocessedSource {
  ...
  final case class NoSourceCode(options: Option[BuildOptions], path: os.Path) extends PreprocessedSource {
  ...
  implicit val ordering: Ordering[PreprocessedSource] =
    new Ordering[PreprocessedSource] {
      def compare(a: PreprocessedSource, b: PreprocessedSource): Int = {
        ...
        if (idxCmp == 0)
          (a, b) match {
            case (a0: NoSourceCode, b0: NoSourceCode) => a0.path.toString.compareTo(b0.path.toString)
