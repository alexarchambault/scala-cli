  ...
  def generateSources(generatedSrcRoot: os.Path): Seq[GeneratedSource] = {
    val generated =
      for (inMemSource <- inMemory) yield {
        os.write.over(generatedSrcRoot / inMemSource.generatedRelPath, inMemSource.generatedContent.getBytes("UTF-8"), createFolders = true)
        (inMemSource.originalPath, inMemSource.generatedRelPath, inMemSource.topWrapperLen)
      }
...
object Sources {
  ...
  final case class InMemory(
    originalPath: Either[String, os.Path],
