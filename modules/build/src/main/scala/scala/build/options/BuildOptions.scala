...
final case class BuildOptions(
  ...
  def crossOptions: Seq[BuildOptions] = {
    val extraScalaVersions = scalaOptions.extraScalaVersions.filter(v => !scalaOptions.scalaVersion.contains(v))
    val sortedExtraScalaVersions = extraScalaVersions.toVector.map(coursier.core.Version(_)).sorted.map(_.repr).reverse
